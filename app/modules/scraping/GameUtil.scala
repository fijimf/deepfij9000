...
object NeutralSiteSolver {
  def apply(games: List[(GameData, Boolean)]): List[(GameData, Boolean, Boolean)] = {
    val locations: List[(String, String)] = games.filter(gd=> gd._1.tourneyInfo.isEmpty && !gd._2).flatMap(g => {
      g._1.location.map(loc => g._1.homeTeamKey -> loc)
    })
    val locLists = locations.foldLeft(Map.empty[String, List[String]])((data: Map[String, List[String]], tup: (String, String)) => data + (tup._1 -> (tup._2 :: data.getOrElse(tup._2, List.empty[String]))))
